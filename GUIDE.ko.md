...
1) 데모 모드가 활성화된 경우 `http://localhost:8081/demo` 접속 후 `Start Demo`를 클릭합니다.

2) 로컬에서 seeder를 실행하고 직접 로그인하려면:
   - 이메일: `hq_manager@pgdemo.com`
   - 비밀번호: `SEEDER_PASSWORD_HQ_MANAGER`에 설정한 값

참고
- 위 계정은 DataSeeder 시딩 단계에서 생성/보정됩니다.
