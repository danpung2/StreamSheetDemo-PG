 ...
 * 결제 트랜잭션 엔티티에 대한 Repository 인터페이스.
 * 
 * Provides data access operations for payment transaction management.
 * 결제 트랜잭션 관리를 위한 데이터 접근 연산을 제공합니다.
 */
@Repository
interface PaymentTransactionRepository : JpaRepository<PaymentTransaction, UUID> {
    ...
    /**
     * Find all payments by merchant with pagination.
     * 특정 업체의 모든 결제를 페이지네이션으로 조회합니다.
     * 
     * @param merchantId the merchant ID / 업체 ID
    ...
    /**
     * Find payments requested within date range.
     * 특정 기간 내에 요청된 결제를 조회합니다.
     * 
    ...
    /**
     * Find payments by merchant and date range.
     * 업체 및 기간별 결제를 조회합니다.
     * 
     * @param merchantId the merchant ID / 업체 ID
     * @param startDate the start of date range / 시작 일시
    ...
    /**
     * Count payments by merchant and status.
     * 업체 및 상태별 결제 건수를 조회합니다.
     * 
     * @param merchantId the merchant ID / 업체 ID
     * @param status the payment status / 결제 상태
     * @return the count of payments / 결제 건수
    ...
    /**
     * Find payments by headquarters (through merchant relationship).
     * 본사별 결제를 조회합니다 (업체 관계를 통해).
     * 
    ...
    /**
     * Find payments by headquarters and date range.
     * 본사 및 기간별 결제를 조회합니다.
     * 
     * @param headquartersId the headquarters ID / 본사 ID
     * @param startDate the start of date range / 시작 일시
    ...
    /**
     * Find payments needing synchronization to MongoDB.
     * MongoDB로 동기화가 필요한 결제를 조회합니다.
     * 
    ...
    /**
     * Find all IDs pending synchronization (for batch job).
     * 동기화 대기 중인 모든 ID를 조회합니다 (배치 작업용).
     * 
     * @param lastSyncTime the last synchronization timestamp / 마지막 동기화 시간
